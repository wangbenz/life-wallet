...
文字输入在所有支持的浏览器可用；Today 与 Life Agent 同时提供渐进增强的语音转文字，识别能力和权限由当前浏览器提供。当前京东云测试环境已通过 Nginx `15173` 发布本版 H5、Spring Boot 后端和 MySQL；DeepSeek 密钥已加密入库并验证真实 Agent 对话，HTTPS 尚未配置，因此 HTTP 环境仍不保证手机语音输入可用。
