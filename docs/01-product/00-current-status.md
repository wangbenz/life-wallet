...
- Today：入口先解释“记录一句生活 → 确认整理结果 → 看见长期变化”，再设置剩余天数估算；首页直接使用天、小时和分钟呈现剩余时间与已记录片段，不再使用“元人生 / 人生币”换算。自然语言记录、Mock 理解、原句对应、遗漏提醒、预览修改、确认保存和准确度反馈保持不变，并说明确认后的记录会进入人生洞察。确认区主动进入视野并避开底部导航。
- Life Agent：从 Today 或指定历史记录进入，用本地 Mock 对话查询、新增、修改或删除记录；保留来源与目标记录上下文，写操作展示差异并等待确认。
...
- 前端 12 项 Mock / 日历测试、TypeScript 和 Vite 生产构建通过。
